...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Dataset folder tag (e.g., depth1-2_n1000_steps1-2_spars0.5-1 or gsm8k_all_nall_seed0).",
    )
    parser.add_argument(
        "--reasoning-models",
        nargs="+",
...
def main() -> None:
    ...
    if args.dataset:
        ...
        min_depth, max_depth, num_samples, steps_range, sparsity_range = parse_tree_dataset_tag(dataset_tag)
        ...
        dataset_tag = path_utils.dataset_tag(min_depth, max_depth, args.num_samples, steps_tag, sparsity=sparsity_range)
        num_samples = args.num_samples
    ...
    if not _already_exists(dataset_path, "dataset", force_set):
        cmd = [
            ...
            str(max_steps),
            "--sparsity",
            str(sparsity_range[0]),
            str(sparsity_range[1]),
            "--dataset",
            dataset_tag,
        ]
