...
def _format_sparsity_value(value: float) -> str:
    text = f"{float(value):.6f}".rstrip("0").rstrip(".")
    return text or "0"


def _format_sparsity_tag(sparsity: Union[float, Tuple[float, float]]) -> str:
    if isinstance(sparsity, tuple):
        if len(sparsity) != 2:
            raise ValueError("sparsity tuple must have length 2")
        smin, smax = float(sparsity[0]), float(sparsity[1])
    else:
        smin = smax = float(sparsity)
    if smin <= 0.0 or smax <= 0.0 or smin > 1.0 or smax > 1.0 or smax < smin:
        raise ValueError("sparsity must satisfy 0 < min <= max <= 1")
    if smin == smax:
        return f"_spars{_format_sparsity_value(smin)}"
    return f"_spars{_format_sparsity_value(smin)}-{_format_sparsity_value(smax)}"
...
def dataset_tag(
    ...
    sparsity: Optional[Union[float, Tuple[float, float]]] = None,
) -> str:
    ...
    if sparsity is not None:
        if isinstance(sparsity, tuple):
            ...
            smin = smax = float(sparsity)
        if not (smin == 1.0 and smax == 1.0):
            sparsity_suffix = _format_sparsity_tag((smin, smax))
    return f"{depth}_{suffix}{steps}{sparsity_suffix}"
...
def parse_tree_dataset_tag(tag: str) -> Tuple[int, int, int, Tuple[int, int], Tuple[float, float]]:
    """Parse traversal dataset tag into (depth range, num_samples, step range, sparsity range)."""

    pattern = re.compile(
        r"^depth(?P<dmin>\d+)(?:-(?P<dmax>\d+))?_n(?P<nsamp>all|\d+)"
        r"(?:_steps(?P<smin>\d+)(?:-(?P<smax>\d+))?)?"
        r"(?:_spars(?P<pmin>\d+(?:\.\d+)?)(?:-(?P<pmax>\d+(?:\.\d+)?))?)?$"
    )
    ...
    pmin = float(match.group("pmin")) if match.group("pmin") is not None else 1.0
    pmax = float(match.group("pmax")) if match.group("pmax") is not None else pmin
    if pmin <= 0.0 or pmax <= 0.0 or pmin > 1.0 or pmax > 1.0 or pmax < pmin:
        raise ValueError(f"Invalid sparsity range in tag: {tag}")
    return dmin, dmax, num_samples, (smin, smax), (pmin, pmax)
...
def default_dataset_path(
    ...
    return dataset_path_from_tag(dataset_tag(min_depth, max_depth, num_samples, num_steps, sparsity=sparsity))
